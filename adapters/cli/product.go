...
func Run(service application.ProductServiceInterface, action string, productId string, productName string, price float64) (string, error) {
	...
	case "create":
		...
		result = fmt.Sprintf("Product ID %s wit the name %s has been created with the price %s and status %s", product.GetID(), product.GetName(), product.GetPrice(), product.GetStatus())
	case "enable":
